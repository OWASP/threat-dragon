...
import { repositoryClear, repositorySelected, repositoryFetch } from '@/store/actions/repository.js';
...
describe('views/RepositoryAccess.vue', () => {
    describe('mounted', () => {
        ...
        it('fetches with default page 1', () => {
            ...
            expect(mockStore.dispatch).toHaveBeenCalledWith(repositoryFetch, { page: 1 });
        });

        it('uses the page from the route query', () => {
            mountComponent({ provider: provider }, { page: 3 });
            expect(mockStore.dispatch).toHaveBeenCalledWith(repositoryFetch, { page: 3 });
        });

        it('does not re-dispatch an already-matched provider', () => {
            mountComponent({ provider: provider }, { page: 1 });
            expect(mockStore.dispatch).not.toHaveBeenCalledWith(providerSelected, provider);
        });
    });
    ...
    describe('onRepoClick', () => {
        ...
        it('dispatches the selected repo', () => {
            expect(mockStore.dispatch).toHaveBeenCalledWith(repositorySelected, repoName);
        });
    ...
    describe('searchQuery watcher (500ms debounce)', () => {
        ...
        it('fires fetch with page 1 after the timeout', async () => {
            ...
            expect(mockStore.dispatch).toHaveBeenCalledWith(repositoryFetch, { page: 1, searchQuery: 'owasp' });
        });
        ...
        it('always uses page 1 regardless of current page', async () => {
            ...
            await wrapper.vm.$nextTick();
            await artificialDelay(500);
            expect(mockStore.dispatch).toHaveBeenCalledWith(repositoryFetch, { page: 1, searchQuery: 'test' });
        });
        ...
        it('debounces: only the last of rapid changes fires', async () => {
            ...
            expect(mockStore.dispatch).toHaveBeenCalledWith(repositoryFetch, { page: 1, searchQuery: 'abc' });
        });
        ...
        it('cancels an earlier timeout when superseded', async () => {
            ...
            expect(mockStore.dispatch).toHaveBeenCalledTimes(1);
            expect(mockStore.dispatch).toHaveBeenCalledWith(repositoryFetch, { page: 1, searchQuery: 'second' });
        });
    });
    ...
    describe('paginate', () => {
        ...
        it('dispatches fetch with given page and current search query', () => {
            ...
            expect(mockStore.dispatch).toHaveBeenCalledWith(repositoryFetch, { page: 2, searchQuery: 'test-repo' });
        });

        it('sends empty search when no filter is active', () => {
            wrapper.vm.searchQuery = '';
            wrapper.vm.paginate(3);
            expect(mockStore.dispatch).toHaveBeenCalledWith(repositoryFetch, { page: 3, searchQuery: '' });
        });

        it('paginates with empty searchQuery when no filter is active', () => {
            wrapper.vm.searchQuery = '';
            wrapper.vm.paginate(5);
            expect(mockStore.dispatch).toHaveBeenCalledWith(repositoryFetch, { page: 5, searchQuery: '' });
        });

        it('preserves the current filter when paginating', () => {
            wrapper.vm.searchQuery = 'owasp';
            wrapper.vm.paginate(4);
            expect(mockStore.dispatch).toHaveBeenCalledWith(repositoryFetch, { page: 4, searchQuery: 'owasp' });
        });
    });
});
