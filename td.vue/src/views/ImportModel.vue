...
export default {
    ...
    methods: {
        ...
        async onOpenClick() {
            if (env.isElectron()) {
                const file = await window.electronAPI.openModel();
                console.warn('Opened model with file path: ', file.path);
                console.warn('Opened model with file text: ', file.text);
                this.tmJson = 'dummy text';
                this.onImportClick();
            } else {
