...
function fixupByContext(suggestions, modelType){
    return suggestions.map((suggestion) => {
        ...
        case linddun.all.linkability:
            if (modelType === 'STRIDE') {
                suggestion.type = stride.all.informationDisclosure;
            } else if (modelType === 'CIA') {
                suggestion.type = cia.confidentiality;
            }
            break;
        case linddun.all.identifiability:
            if (modelType === 'STRIDE') {
                suggestion.type = stride.all.spoofing;
            } else if (modelType === 'CIA') {
                suggestion.type = cia.confidentiality;
            }
            break;
        case stride.all.repudiation:
            if (modelType === 'LINDDUN') {
                suggestion.type=linddun.all.nonRepudiation;
            } else if (modelType === 'CIA') {
                suggestion.type = cia.integrity;
            }
            break;
        case linddun.all.detectability:
            if (modelType === 'STRIDE') {
                suggestion.type = stride.all.informationDisclosure;
            } else if (modelType === 'CIA') {
                suggestion.type = cia.confidentiality;
            ...
            break;
        case linddun.all.disclosureOfInformation:
            if (modelType === 'STRIDE') {
                suggestion.type = stride.all.informationDisclosure;
            } else if (modelType === 'CIA') {
                suggestion.type = cia.confidentiality;
            ...
            break;
        case linddun.all.unawareness:
            if (modelType === 'STRIDE') {
                suggestion.type = stride.all.repudiation;
            } else if (modelType === 'CIA') {
                suggestion.type = cia.integrity;
            }
            break;
        case stride.all.tampering:
            if (modelType === 'LINDDUN') {
                suggestion.type = linddun.all.nonCompliance;
            } else if (modelType === 'CIA') {
                suggestion.type = cia.integrity;
            }
            break;
...
export const GetContextSuggestions = (element, model) => {
    ...
    if ((element.type === 'tm.Actor' && element.providesAuthentication)) {
        ...
        suggestions.push({
            ...
            type: linddun.all.identifiability,
    ...
    if ((element.type === 'tm.Flow' && element.isPublicNetwork && !element.isEncrypted)) {
        suggestions.push({
            ...
            type: linddun.all.disclosureOfInformation,
    ...
    if ((element.type === 'tm.Process')) {
        ...
        suggestions.push({
            ...
            type: linddun.all.detectability,
    ...
    if ((element.type === 'tm.Store' && element.isALog)) {
        suggestions.push({
            title: 'Log contains sensitive data',
            type: linddun.all.disclosureOfInformation,
