...
> **Note:** The static fallback is **one‑way** (`pt` → `pt-BR`). `pt-BR` does not fall back to
European Portuguese (`pt`) by default, unless you add it to the `fallbackLocale` object.
