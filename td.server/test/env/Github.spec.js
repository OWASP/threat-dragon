...
describe('env/Github.js', () => {
    ...
    it('has the optional property CLIENT_ID', () => {
        const isRequired = githubEnv.properties.find((x) => x.key === 'CLIENT_ID').required;
        expect(isRequired).to.be.false;
    });

    it('has the optional property CLIENT_SECRET', () => {
        const isRequired = githubEnv.properties.find((x) => x.key === 'CLIENT_SECRET').required;
        expect(isRequired).to.be.false;
    });

    it('has the optional property SCOPE', () => {
        const isRequired = githubEnv.properties.find((x) => x.key === 'SCOPE').required;
        expect(isRequired).to.be.false;
    });

    it('has a default value for property SCOPE', () => {
        const value = githubEnv.properties.find((x) => x.key === 'SCOPE').defaultValue;
        expect(value).to.equal('public_repo');
    });

    it('has the optional property ENTERPRISE_HOSTNAME', () => {
        const isRequired = githubEnv.properties.find(
    ...
    it('has the optional property ENTERPRISE_PORT', () => {
        const isRequired = githubEnv.properties.find((x) => x.key === 'ENTERPRISE_PORT').required;
        expect(isRequired).to.be.false;
    });

    it('has a default value for property ENTERPRISE_PORT', () => {
        const value = githubEnv.properties.find((x) => x.key === 'ENTERPRISE_PORT').defaultValue;
    ...
    it('has a default value for property ENTERPRISE_PROTOCOL', () => {
        const value = githubEnv.properties.find(
            (x) => x.key === 'ENTERPRISE_PROTOCOL'
        ).defaultValue;
    ...
    it('has the optional property USE_SEARCH', () => {
        const isRequired = githubEnv.properties.find((x) => x.key === 'USE_SEARCH').required;
        expect(isRequired).to.be.false;
    });

    it('has a default value for property USE_SEARCH', () => {
        const value = githubEnv.properties.find((x) => x.key === 'USE_SEARCH').defaultValue;
        expect(value).to.be.false;
    });

    it('has the optional property SEARCH_QUERY', () => {
        const isRequired = githubEnv.properties.find((x) => x.key === 'SEARCH_QUERY').required;
        expect(isRequired).to.be.false;
    });
