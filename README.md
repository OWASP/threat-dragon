...
2. [**A desktop application**](https://github.com/mike-goodwin/owasp-threat-dragon-desktop): This is based on [Electron](https://electron.atom.io/). There are released build images available for both Windows and OSX (a Linux install package will follow at some point). For the desktop varian the models are stored on the local filesystem rather than a remote repository.
